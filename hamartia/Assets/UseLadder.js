...
public var climbMaxAnimationSpeed : float = 1.0;
private var isFalling : int = 0;
...
// }
//function PlayerTouchLadder(other : Collider) {
		Debug.Log("PlayerTouchLadder");
...
//function PlayerExitLadder(other : Collider) {
	Debug.Log("PlayerExitLadder");
	if(currentLadder!=null && other.tag == "fallingLadder"){
		if(currentLadder.isFalling() == 1){
		//Falling ladder
			//mainCamera.transform.parent = currentLadder;
			currentLadder.RotateSelf();
		}
	}
	else{
		if(other.tag == "LadderLandingPad") {
...
function LatchLadder (latchedLadder : GameObject, collisionWaypoint : Collider) {
	...
	Debug.Log("latch ladder, on ladder");
...
function LadderFixedUpdate () {
	...
	if (Input.GetButton("Jump")) {
		UnlatchLadder();
		gameObject.SendMessage("FixedUpdate", null, SendMessageOptions.DontRequireReceiver);
		return;
	}
	_animation[climbAnimation.name].speed = Mathf.Clamp(controller.velocity.magnitude, 0.0, climbMaxAnimationSpeed);
